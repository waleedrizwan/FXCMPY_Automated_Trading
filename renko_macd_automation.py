# -*- coding: utf-8 -*-
"""
Created on Wed Sep  2 13:35:07 2020
...
# token for the demo trading account
token_path = r'C:\Users\walee\Desktop\FXCMPY.txt'
# establish API connection and define trade parameters
...
# max capital allocation for all currency pairs
pos_size = 10
...
def renko_merge(DF):
    '''
    function merges renko df with original ohlc df
    '''
...
def trade_signal(MERGED_DF, l_s):
    '''
    Generates trade signal based on
    current position in the asset,
    combined with Renko MACD analysis 
    '''
    
    signal = ""
    df = copy.deepcopy(MERGED_DF)
    
    # No position has been taken in the asset
    if l_s == "":
...
def main():
    try:
        ...
        for currency in pairs:
            long_short = ""
            # if there is atleast 1 current open position
            if len(open_pos)>0:
            ...
            if signal == "Buy":
                con.open_trade(symbol= currency,is_buy=True, is_in_pips=True, amount=pos_size, time_in_force= 'GTC',stop=-8,
                               trailing_step =True, order_type="AtMarket")
                print("New long position initiated for: ",currency)
            elif signal == "Sell":
                con.open_trade(symbol=currency, is_buy=False,is_in_pips=True,
                               amount=pos_size,time_force = 'GTC', stop=-8, trailing_step=True, order_type='AtMarket')
                print("New short position for, ", currency)
            
            elif signal == "Close":
                con.close_all_for_symbols(currency)
        ...
        print("error encountered iteration skipped")
...
# After trades have been made, closes positions and ends script
for currency in pairs:
